...
Here is an early concept artboards of a potential mobile app:

![handshake artnpard](handshake-mobile-art-board.png)

## Overview of the fundamental concepts

In handshake, keys never pass over the wire. This is possible because each chat participant shares 96 ***entropy*** bytes encoded into the QR code as part of the handshake. These unique bytes, plus a strict sort order enforced by the initiator of the chat, ensures that all lookup hashes and keys are able to be deterministically generated by all participants.
...
TODO:
- what is the structure of a handshake?
- what are strategies?
- How is a complete handshake performed?
